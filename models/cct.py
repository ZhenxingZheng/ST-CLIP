...
import sys
from models.global_reasoning_unit import GloRe_Unit_1D
sys.path.append("../../")
...
class CrossFramelAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, droppath=0., T=0, ):
        ...
        # self.message_gcn = GloRe_Unit_1D(d_model, d_model/16, True)
        self.message_gcn = temp_conv1d(embed_dim=d_model)
        self.message_pos_embed = nn.Parameter(torch.zeros(T, d_model))
    ...
    def forward(self, x):
        ...
        msg_token = msg_token + self.message_pos_embed.unsqueeze(0)

        msg_token = self.message_gcn(msg_token)
        # x[0] = msg_token

        x = torch.cat([x, msg_token.unsqueeze(0)], dim=0)
        x = x.view(l+1, -1, d)
