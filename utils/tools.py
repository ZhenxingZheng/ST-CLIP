...
def epoch_saving(config, epoch, model,  max_accuracy, optimizer, lr_scheduler, logger, working_dir, is_best):
    ...
    save_path = os.path.join(working_dir, f'ckpt_epoch_final.pth')
...
def load_checkpoint_model(model_path, model, optimizer, lr_scheduler, logger):
    if os.path.isfile(model_path):
        logger.info(f"==============> Resuming form {model_path}....................")
        ...
        try:
            logger.info(f"=> loaded successfully '{model_path}' (epoch {checkpoint['epoch']})")
            del checkpoint
...
def load_checkpoint(model_path, model, optimizer, lr_scheduler, logger):
    if os.path.isfile(model_path):
        logger.info(f"==============> Resuming form {model_path}....................")
        checkpoint = torch.load(model_path, map_location='cpu')
