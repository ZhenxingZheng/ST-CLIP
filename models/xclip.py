...
class XCLIP(CLIP):
    ...
    def encode_video(self, image):
        ...
        img_features = img_features.mean(1)

        # video_features = self.mit(cls_features)  # line 135

        return cls_features, img_features
    ...
    def forward(self, image, text):
        ...
        video_features = img_features.mean(dim=1, keepdim=False)
...
def build_model(state_dict: dict, T=8, droppath=0., use_checkpoint=False, logger=None, prompts_alpha=1e-1,
                prompts_layers=2, use_cache=True, mit_layers=4, ):
